...
| Large histories and global search remain complete | Multiple-page sessions and messages, including newest messages beyond the old fetch cap | Partial: BE-005 global finder and BE-010 message history preserve server cursors, including empty pages. Chats open at the newest page and load older messages explicitly. Scoped session inventory remains capped and incomplete. The pinned v1 server has an upstream global timestamp-tie limitation documented in the backlog. |
