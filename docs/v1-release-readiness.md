...
| Daily mobile flows preserve input and show current server truth | New-chat commands, refresh failures, connection edits during probes, review selection, per-session model/agent state, and resolved requests | Partial: FE-005/006/009 implemented in cycle 03; review selection/patch retention and Files Back in cycle 04. V2 per-session model/variant/agent synchronization and explicit offline snapshots implemented in cycle 07. Resolved-request handling, staged revert and remaining linked workflows remain open; final cross-client/device verification is pending. |
...
2. Finish staged revert parity and verify the implemented pagination and
