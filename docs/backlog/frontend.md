...
| [#55 — staged revert, commit, clear](https://github.com/Eslamasabry/opencode-mobile-next/issues/55) | `chat_screen.dart::_revertLast` presents a one-step Revert; `api2/gateway_operations.dart::revertSession` uses `/revert/stage` with `files: true`, and Restore uses `/revert/clear`. The code does request file rollback; the missing part is explicit temporary state, affected-file preview, commit, and clear. Add the persistent staged-state banner and separate actions, preserving v1's existing flow. Reuse `test/api2_revert_test.dart` and confirm the stage/commit contract once against the supported beta before claiming complete v2 revert. |
| [#53 — cross-client model/agent state](https://github.com/Eslamasabry/opencode-mobile-next/issues/53), [#57 — unread/viewed sessions](https://github.com/Eslamasabry/opencode-mobile-next/issues/57) | `lib/api2/events.dart` decodes selected/viewed events, but the visible session state still lacks the corresponding complete cross-client workflow. Keep these as protocol-plus-UI slices: hydrate and reconcile the matching session only; show a text/semantic unread state; mark viewed only when the idle chat is foregrounded and honor the issue's privacy opt-out. Existing per-session selection and Running work improvements must remain intact. |
