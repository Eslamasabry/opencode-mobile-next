...
| [#29 — pin/unarchive](https://github.com/Eslamasabry/opencode-mobile-next/issues/29), [#47 — persistent prompt stash](https://github.com/Eslamasabry/opencode-mobile-next/issues/47), [#25 — transcript find](https://github.com/Eslamasabry/opencode-mobile-next/issues/25) | Cycle 16 implements persisted local pins with a top section, row/context-menu actions, older-page hydration, storage recovery and profile cleanup. Unarchive remains unavailable through the pinned HTTP contracts: v1 zero stays archived in the database and null is rejected; v2 exposes no archive-write route. See `../verification/session-pins-and-unarchive.md`. Persistent stash remains separate from existing session drafts/prompt reuse. Timeline already searches loaded message text and jumps to a message (`chat/timeline_sheet.dart`); #25 should add match navigation/highlighting and honest search coverage, not another search-only sheet. Follow the existing issue acceptance criteria and reuse these entry points. |
