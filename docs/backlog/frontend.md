...
| [#54 — aggregate usage](https://github.com/Eslamasabry/opencode-mobile-next/issues/54), [#48 — JSON export](https://github.com/Eslamasabry/opencode-mobile-next/issues/48) | Usage remains per session, and `_exportTranscript` exports rendered Markdown. Add a secondary usage screen with range/project scope and a redacted server JSON option in the existing export action. Keep unsupported-server actions hidden. These remain missing parity workflows. |
