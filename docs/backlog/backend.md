...
| Export implemented; import pending | **Complete backup and transfer:** [#48](https://github.com/Eslamasabry/opencode-mobile-next/issues/48), cycle 14 adds complete server JSON beside loaded Markdown, redaction on by default, cancellation and retry. Live verification confirms redaction replaces conversation text; users must turn it off to preserve original text in a backup. | Import remains separate: validate the transfer payload, review the destination location, handle the live-confirmed 409 conflict, and retain the original on failure. See `../verification/export-beta-18600.md`. Captured v1 has no equivalent transfer routes. |
