...
class ControlApi {
  ...
  Future<Response<bool>> appLog({
    String? directory,
    String? workspace,
    AppLogRequest? appLogRequest,
    ...
    try {
      if (appLogRequest != null) {
        _bodyData = jsonEncode(appLogRequest);
      }
    ...
    try {
      _response = await requestOpenCode<Object>(
        _dio,
        _path,
        includeBody: appLogRequest != null,
        bodyData: _bodyData,
        options: _options,
        queryParameters: _queryParameters,
        cancelToken: cancelToken,
  ...
  /// Returns a [Future] containing a [Response] with a [bool] as data
  /// Throws [DioException] if API call or serialization fails
  Future<Response<bool>> authRemove({
    ...
    const _operationId = r'auth.remove';
    final _path = r'/auth/{providerID}'.replaceAll(
      '{'
      r'providerID'
      '}',
